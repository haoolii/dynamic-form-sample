// ============ 型別定義 ============
import { Component, signal, Input, Output, EventEmitter, inject, DestroyRef, ChangeDetectionStrategy, input, effect } from '@angular/core';
import { CommonModule } from '@angular/common';
import { ReactiveFormsModule, FormControl, FormGroup, FormArray, Validators } from '@angular/forms';
import { takeUntilDestroyed, toObservable, toSignal } from '@angular/core/rxjs-interop';
import { debounceTime, delay, distinctUntilChanged, observeOn, startWith, switchMap } from 'rxjs/operators';
import { Injectable } from '@angular/core';
import { asyncScheduler, of, queueScheduler, scheduled, Subscription } from 'rxjs';
...
export class RuleTypeFormComponent {
  ...
  typeForm = input<FormGroup<RootRuleForm> | null>(null);
  formValue = signal<any>(null);
  formValue2 = toSignal(
  toObservable(this.typeForm).pipe(
    switchMap(form =>
      form
  ...
  constructor() {
    ...
    //     startWith(scheduled([form.value], asyncScheduler)),
    //     delay(0)
    //   ).subscribe(v => {
    //     console.log('v', v)
  ...
  getOrGroups(): FormGroup<OrGroupForm>[] {
    ...
    if (!typeForm) return [];
    return typeForm.controls.children.controls as FormGroup<OrGroupForm>[];
  }
}
